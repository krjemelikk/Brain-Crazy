...
public class GameController : MonoBehaviour
{
   ...
   [Sirenix.OdinInspector.ReadOnly] public int capingInter;
   [Sirenix.OdinInspector.ReadOnly] public int capingVideoReward;
   [Sirenix.OdinInspector.ReadOnly] public int capingBaner;
   ...
   public void SetNotify()
   {
      ...
      long delay1 = timeCurrent > time1
         ? (long) (time1.TotalSeconds - timeCurrent.TotalSeconds) + 24 * 60 * 60
         : (long) (time1.TotalSeconds - timeCurrent.TotalSeconds);
      long delay2 = timeCurrent > time2
         ? (long) (time2.TotalSeconds - timeCurrent.TotalSeconds) + 24 * 60 * 60
         : (long) (time2.TotalSeconds - timeCurrent.TotalSeconds);
      float timedelay =
         RemoteConfigController.GetFloatConfig(StringHelper.ConfigFirebase.CONFIG_PUSH_NOTI_OFFLINE, 24f);
      LocalNotification.SendRepeatingNotification(ID_NOTIFY_PLAY_GAME1, (long) (delay1 * 1000),
         (long) (24 * 60 * 60 * 1000), "Brain Crazy IQ Challenge: Can you pass it ?",
         string.Format(Localization.Get(keyLocalize[UnityEngine.Random.Range(0, keyLocalize.Length)]),
            DataManager.CurrentLevel), new Color32(0xff, 0x44, 0x44, 255));
      LocalNotification.SendRepeatingNotification(ID_NOTIFY_PLAY_GAME2, (long) (delay2 * 1000),
         (long) (24 * 60 * 60 * 1000), "Brain Crazy IQ Challenge: Can you pass it ?",
         string.Format(Localization.Get(keyLocalize[UnityEngine.Random.Range(0, keyLocalize.Length)]),
            DataManager.CurrentLevel), new Color32(0xff, 0x44, 0x44, 255));
      LocalNotification.SendRepeatingNotification(ID_NOTIFY_PLAY_GAME3, (long) (timedelay * 60 * 60 * 1000),
         (long) (timedelay * 60 * 60 * 1000), "Brain Crazy IQ Challenge: Can you pass it ?",
         string.Format(Localization.Get(keyLocalize[UnityEngine.Random.Range(0, keyLocalize.Length)]),
            DataManager.CurrentLevel), new Color32(0xff, 0x44, 0x44, 255));
   }
   ...
   public void StartLevel(int _level)
   {
      ...
      else
      {
         ...
         var level = dataContains.dataLevels.GetLevel(DataManager.CurrentLevel);
         string nameLevel = level.NAME_LEVEL;
         ...
         if (level.ID == 27)
         {
            Instance.UseHint(0, ReasonUseHint.SkipLevel); 
         }
      }
   ...
   public void SetUpdate(UpdateStatus updateData)
   {
      ...
      if (updateData.version > currentVersion)
      {
         ...
         {
            if (!DataManager.IsDontShowAgainPopupUpdate(updateData.version) &&
                TimeManager.CaculateTime(DataManager.LastTimeShowPopupUpdate, System.DateTime.Now) >=
                updateData.offset_show)
            {
               //Show Popup
